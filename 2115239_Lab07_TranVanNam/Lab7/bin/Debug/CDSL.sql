...
VALUES (N'Hóa đơn', 5, 150000, 0.05, 0, 1, NULL, N'lgcong')

INSERT INTO [dbo].[Bills] ([Name], [TableID], [Amount], [Discount], [Tax], [Status], [CheckoutDate], [Account])
VALUES (N'Hóa đơn chưa thanh toán', 5, 150000, 0.05, 0, 1, NULL, N'ngaptt')
...
CREATE PROCEDURE USP_ThemKhachTaiKhoan
   @AccountName nvarchar(100),
    @Password NVARCHAR(2000),
    @FullName nvarchar(1000),
    @Email nvarchar(1000),
...
END
--theem1 kh
-- thủ tục xem hóa đơn của 1 khoảng thười gian
...
BEGIN
    SELECT *
    FROM Bills
    WHERE [CheckoutDate] BETWEEN @StartDay AND @EndDay;
END
EXEC Bills_GetByDate '2023-01-01', '2023-12-31';

-- thủ tục xem chi tiết 1 hóa đơn

create proc BillDetail_GetById
	@id int
as
begin
